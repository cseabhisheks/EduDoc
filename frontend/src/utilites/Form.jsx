    ...
    return (<>
      <div className="flex items-center justify-center h-[100vh] bg-indigo-100">
          <div className="bg-white max-h-[100vh] border-2 w-[80vw] md:w-[30vw] p-4 md:p-8 flex flex-col gap-2 rounded-xl shadow-2xl ">
            ...
            {/* field */}
            {fields.map((e, idx) => (
                <label key={idx} htmlFor="email" >
                    <b className="capitalize text-xs"><span>{e.label}</span></b>
                    <input className="mt-1 border-2 bg-gray-200 text-xs h-[20px] md:h-[30px] w-full px-2 capitalize rounded-md" type={e.type} name={e.label} id={e.label} placeholder={e.placeholder} />
                </label>
            ))}
            {/* submit button */}
